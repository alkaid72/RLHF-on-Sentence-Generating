...
The recorder in the training process has a function similar to SummaryWriter, but SummaryWriter needs to rely on tensorboard and browser for visualization.
The tool relies on matplotlib to store static local images, which is convenient for the server to quickly view the training results.
...
class iSummaryWriter(object):
    ...
    def create_params_dict(self, params: list) -> dict:
        ...
        Create a monitoring variable dictionary based on the list of variable names that need to be recorded.
    ...
    def update_ax_list(self) -> None:
        ...
        According to the current monitoring variable dictionary, assign a plot area to each variable.
        """
        # * 重新计算每一个变量对应的图幅索引
