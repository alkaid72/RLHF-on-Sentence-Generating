...
Use the idea of Reward Model in ChatGPT to train an RM, because it is a scoring model, so use BERT (not GPT) model training.
...
def evaluate_model(model, data_loader):
    """
    在测试集上评估当前模型的训练效果。
    Evaluate the training effect of the current model on the test set
